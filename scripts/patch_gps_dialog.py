...
# No region slicing - operates on full source
...
# This is a multi-line pattern from "String timeStr" to end of if block closing brace
old_block = (indent + "String timeStr = '';\n" +
             indent + "if (ts is Timestamp) {\n" +
             inner  + "final dt = ts.toDate();\n" +
             inner  + "timeStr =\n")

# Find old_block in full source (after anchor)
region = src[timestr_idx:]
blk_idx = region.find(old_block)
if blk_idx < 0:
    print('ERROR: old_block not found!')
    print('Looking for:', repr(old_block))
    # Debug: show what is at timestr position
    print('Actual content:', repr(src[timestr_idx:timestr_idx+200]))
    sys.exit(1)

abs_blk_idx = timestr_idx + blk_idx
